...
def r_squared(y, estimated):
    ...
    SSR = ((estimated-y)**2).sum()
    mean = y.sum()/len(y)
    SSE = ((y - mean)**2).sum()
    return float(1-SSR/SSE)
...
def evaluate_models_on_training(x, y, models):
    ...
    Returns:
        None
    """
    xVals,yVals = np.array(x),np.array(y)
    for m in models:
        pylab.plot(xVals,yVals,"bo",label = "Measured Points")
        pylab.title("Temperature vs Year")
        pylab.xlabel("Year")
        pylab.ylabel("Temperature")
        estYvals = pylab.polyval(m,xVals)
        rsqu = round(r_squared(yVals, estYvals),3)  
        pylab.plot(xVals, estYvals, "r",label = 'R^2 =' + str(rsqu))
